...
{
  $newrow="<td><div class=cart-img><img   src=uploads/".$json_output["datavalues"][$a]["image"]." alt=The wedding budget></div></td><td>".$json_output["datavalues"][$a]["item"]."</td><td>".$json_output["datavalues"][$a]["provider"]."</td><td>".$json_output["datavalues"][$a]["location"]."</td><td><b>".$json_output["datavalues"][$a]["price"]."</b></td><td><div class=cart-action>  ".$othervendorscount." </div></td>";
...
echo '<tr style="width:100%;"><td><div  class="cart-img"> <img src="uploads/'.$json_output["datavalues"][$a]["image"].'" /></div></td><td>'.$json_output["datavalues"][$a]["provider"].'</td><td>'.$json_output["datavalues"][$a]["location"].'</td><td>'.$json_output["datavalues"][$a]["price"].'</td> <td><a  class="btn btn-default" onclick="hidebudgetmodal();'. $viewdetailsfunction.'"   data-target="#providermodal" >  <i  class="fa fa-play"></i> View Details</a></td> <td><input onchange="changebudget(\''.$maintablerowid.'\',\''.urldecode($newrow).'\',\''.$itembudget.'\',\''.$json_output["datavalues"][$a]["price"].'\',\'box'.$a.'_'.$json_output["datavalues"][$a]["item"].'_'.$json_output["datavalues"][$a]["price"].'\',\''.$json_output["datavalues"][$a]["item"].'\');" type="radio" name='.$json_output["datavalues"][$a]["item"].' id="box'.$a.'"  '.$radiochecked.'></td></tr>';
